...
const double thick_tr = HEIGHT * 1.0;
const double thick_tl = HEIGHT * 1.0;
const double thick_bl = HEIGHT * 1.0;
const double thick_br = HEIGHT * 1.0;
const double gv_taper_top = 1.0;
const double gv_taper_bot = 1.0;
const double radius_top = 0.5;
const double radius_bot = 2.0;
//const double thick_rnd = 0.25;
const double thick_rnd = 0.0;
// bounding box width and height
double bb_width=-1, bb_height=-1, bb_radius=-1, bb_angle=-1;
...
{
	fprintf( stderr,
		 "Usage: genplane [-]<n> <m> <s_type> <radius> <height> <angle>\n"
		 "\n"
...
void parse_arguments( int argc, char ** argv)
{
	...
	if( 1 != sscanf( argv[ag], "%lf", & bb_radius)) usage();
	ag ++;
	if( 1 != sscanf( argv[ag], "%lf", & bb_height)) usage();
	ag ++;
	if( 1 != sscanf( argv[ag], "%lf", & bb_angle)) usage();
	ag ++;

	bb_width = 2 * M_PI * bb_radius * (bb_angle/360.0);
}
...
//		execute( "tail +2 < /tmp/rndpts > /tmp/rndpts2");
	...
	// map the points on a cylinder
	for( long i = 0 ; i < n_nodes ; i ++) {
		double x = nodes[i]-> x;
		double y = nodes[i]-> y;
		double z = nodes[i]-> z;
		double iso_y = (bb_height/2-nodes[i]-> y)/bb_height;
		double taper = (1-iso_y) * radius_top + iso_y * radius_bot;
		double curr_radius = taper * bb_radius;
		nodes[i]-> x = (curr_radius + z) * sin( x / bb_radius);
		nodes[i]-> y = (curr_radius + z) * cos( x / bb_radius);
		nodes[i]-> z = y;
	}
	...
	for( long i = 0 ; i < n_nodes ; i ++) {
		// no taper
		// double taper = 1;
		double iso_y = (bb_height/2-nodes[i]-> z)/bb_height;
		double taper = (1-iso_y) * gv_taper_top + iso_y * gv_taper_bot;
		nodes[i]-> gx = nodes[i]-> x * taper;
		nodes[i]-> gy = nodes[i]-> y * taper;
		nodes[i]-> gz = 0;
// 		nodes[i]-> gx = nodes[i]-> x;
// 		nodes[i]-> gy = nodes[i]-> y;
// 		nodes[i]-> gz = nodes[i]-> z;
	}
