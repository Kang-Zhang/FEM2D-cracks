...
static void usage( void)
{
	fprintf( stderr,
		 ...
		 "     -bb    polygonal representation of the bounding box.\n"
		 "            First number specifies the number of points,\n"
		 "            then the points are listed in counter-clkwise\n"
		 "            directions. Default is:\n"
		 "                -bb 4 -0.5 -0.5 0.5 -0.5 0.5 0.5 -0.5 0.5\n"
...
//		fprintf( stderr, "Processing '%s'\n", argv[ag]);
		...
		if( strcmp( argv[ag], "-n") == 0) {
			...
			// width of the patch
			if( ag + 2 >= argc ||
			    1 != sscanf( argv[ag+1], "%lf", & w) ||
			    1 != sscanf( argv[ag+2], "%lf", & h) ||
			    w <= 0 ||
			    h <= 0)
				usage();
			bbox.reset();
		...
		else if( strcmp( argv[ag], "-bb") == 0) {
			fprintf( stderr, "ag=%d argc=%d\n", ag, argc);
			long n;
			if( ag + 1 >= argc ||
			    1 != sscanf( argv[ag+1], "%ld", & n) ||
			    n < 3)
				usage();
			fprintf( stderr, "n=%ld\n", n);
			ag += 2;
			...
			for( long i = 0 ; i < n ; i ++) {
				double x, y;
				if( 1 != sscanf( argv[ag], "%lf", & x) ||
				    1 != sscanf( argv[ag+1], "%lf", & y))
					usage();
				bbox.add( x, y);
				ag += 2;
			}
	...
	fprintf( stderr, "BBOX: min/max = [%f,%f] - [%f,%f]\n",
...
//		execute( "tail +2 < /tmp/rndpts > /tmp/rndpts2");
	...
	for( long i = 0 ; i < n_nodes ; i ++) {
		...
		//		double bb_height = bbox.maxy - bbox.miny;
		//		nodes[i]-> gx = nodes[i]-> x * (bb_height/2-nodes[i]-> y);
//		nodes[i]-> gx = nodes[i]-> x * (0.5-nodes[i]-> y);
