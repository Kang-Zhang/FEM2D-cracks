 ...
 */
/* Modified by Pavol Federl
 *
 * - incorporating the code to work with my FEM program for cracking
 *
 */

#include <stdio.h>
#include <math.h>
#include "nullspace.hh"
...
/*
	if( ns_debug_flag)
	{
		fprintf( stderr, "null: R(1) =\n");
		for( long i = 0 ; i < n ; i ++) {
			for( long j = 0 ; j < n ; j ++)
				fprintf( stderr, "%30.20f ", R(i,j));
			fprintf( stderr, "\n");
		}
		fprintf( stderr, "r[]= ");
		for( long i = 0 ; i < n ; i ++)
...
*/
	...
	for (i = 0; i < n; i++) {
		...
		if (max < eps)
			break;	/* Consider this and all subsequent pivots
				 * to be zero */
		...
		for (k = 0; k < n; k++) { /* Row elimination */
			if (k == kk)
				continue; /* Don't do a thing to the pivot
					   * row */
			temp = R(k, jj);
			R(k, jj) = 0.0;
			for (j = 0; j < n; j++) {
				R(k, j) -= temp * R(kk, j); /* Subtract row
							     * kk from row
							     * k */
				if (fabs(R(k, j)) < eps)
					R(k, j) = 0.0; /* Flush to zero if
							* too small */
			}
		}
	}
...
*/
	...
	for (j = 0; j < n; j++) { /* For all columns... */
		while ((c[j] >= 0) && (j != c[j])) {
			...
			t = c[j]; /* Twiddle until pivots are on the
				   * diagonal */
			c[j] = c[t];
			c[t] = t;
		}
